...
import os
import streamlit as st
from loaders import load_csv_folder
from qa import build_index, make_rag_chain, CHROMA_DIR

st.set_page_config(page_title="Delivery Intelligence (LangChain + RAG)", page_icon="🚚")

st.title("🚚 Delivery Intelligence — LangChain + RAG")
...
with st.sidebar:
    ...
    st.markdown("**Index directory:** `data/*.csv`")
    st.markdown(f"**Vector store:** `{CHROMA_DIR}`")

st.subheader("1) Build / Refresh Index")
if st.button("Index CSVs"):
    docs = load_csv_folder("data")
    if not docs:
        st.warning("No CSV files found in ./data. Add files and try again.")
    else:
        build_index(docs)
        st.success(f"Indexed {len(docs)} rows. You can ask questions now!")

st.subheader("2) Ask a Question")
question = st.text_input("Try: 'Which project is at risk this week and why?'")

if st.button("Answer") and question.strip():
    chain = make_rag_chain()
    with st.spinner("Thinking..."):
        result = chain.invoke(question)
    # result is an AIMessage; print content only
    st.markdown("**Answer:**")
    st.write(getattr(result, "content", str(result)))
